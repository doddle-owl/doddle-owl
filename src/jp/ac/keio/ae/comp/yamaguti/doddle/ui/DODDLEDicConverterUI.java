...
public class DODDLEDicConverterUI extends JDialog implements ActionListener {
    ...
    private DictionaryType getDicType() {
        ...
        } else if (jpnwnRadioButton.isSelected()) { return DictionaryType.JPNWN; }
        return DictionaryType.EDR;
    }
        ...
        new Thread() {
            ...
            public void convertOWL(DictionaryType dicType) {
                if (dicType == DictionaryType.JPNWN) {
                    ...
                    JPNWN2DODDLEDicConverter.readConceptData();
                    JPNWN2DODDLEDicConverter.writeOWLConceptData(ontModel, DODDLEConstants.JPN_WN_URI);
                    JPNWN2DODDLEDicConverter.saveOntology(ontModel, dicType + ".owl");
                    Model treeOntModel = ModelFactory.createDefaultModel();
                    JPNWN2DODDLEDicConverter.writeOWLTreeData(treeOntModel, DODDLEConstants.JPN_WN_URI);
            ...
            public void run() {
                ...
                if (dicType == DictionaryType.JPNWN) {
                    ...
                    boolean isEnable = EDR2DoddleDicConverter.setEDRDicPath(orgDicPathField.getText(), dicType);
                    if (!isEnable) {
                        initProgressBar("Error: FileNotFound");
                        return;
                    }
