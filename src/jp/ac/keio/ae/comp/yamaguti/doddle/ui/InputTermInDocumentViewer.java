 ...
 */
public class InputTermInDocumentViewer extends JPanel implements MouseListener, ActionListener, HyperlinkListener,
        KeyListener {
    ...
    private void addUserDefinedWord(String word) {
        ...
        if (selectedDoc.getLang().equals("ja")) {
            try {
                ...
                List<Token> tokenList = tagger.analyze(word);
                for (Token token : tokenList) {
                    basicWord += token.getMorpheme().getBasicForm();
                }
        ...
        InputTermSelectionPanel inputTermSelectionPanel = DODDLE.getCurrentProject().getInputTermSelectionPanel();
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == documentList && e.getModifiers() == InputEvent.BUTTON1_MASK) {
            ...
            if (0 < documentList.getModel().getSize()) {
                ...
                documentAreaBorder.setTitle(Translator.getTerm("InputDocumentArea") + " (" + (lineNum + 1) + "-"
                        + (lineNum + DISPLAY_LINE_NUM) + ")");
    ...
    private boolean isCompoundWord(String pos) {
        return pos.indexOf(InputDocumentSelectionPanel.COMPOUND_WORD_JA) != -1
                || pos.toLowerCase().indexOf(InputDocumentSelectionPanel.COMPOUND_WORD_EN) != -1;
    }
    ...
    private List<String> getJaBasicWordList(String text) {
        ...
        try {
            ...
            for (Token token : tokenList) {
                basicWordList.add(token.getMorpheme().getBasicForm());
            }
        } catch (IOException ioe) {
            ioe.printStackTrace();
        }
    ...
    private String highlightEnText(String text) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            InputTermSelectionPanel inputTermSelectionPanel = DODDLE.getCurrentProject().getInputTermSelectionPanel();
            InputDocumentSelectionPanel docSelectionPanel = DODDLE.getCurrentProject().getDocumentSelectionPanel();
    ...
    private String highlightJaText(String text) {
        ...
        try {
            ...
            for (Token token : tokenList) {
                ...
                InputTermSelectionPanel inputTermSelectionPanel = DODDLE.getCurrentProject()
        ...
        for (TermInfo info : termInfoSet) {
            ...
            for (String pos : posSet) {
                if (isCompoundWord(pos) || isUserDefinedWord(pos)) {
                    List<String> compoundWordList = new ArrayList<String>();
                    String[] words = word.split("\\s+");
                    for (int i = 0; i < words.length; i++) {
                        String basic = "";
            ...
            for (String pos : posSet) {
                if (isCompoundWord(pos) || isUserDefinedWord(pos)) {
                    ...
                    try {
                        ...
                        List<Token> tokenList = tagger.analyze(word);
                        for (Token token : tokenList) {
                            compoundWordList.add(token.getMorpheme().getBasicForm());
                        }
                    } catch (IOException ioe) {
                        ioe.printStackTrace();
                    }
    ...
    private String highlightEnCompoundWord(String text) {
        ...
        for (int i = 0; i < basicStringList.size(); i++) {
            ...
            if (0 < compoundWordSize) {
                ...
                builder.append("<font color=\"" + CORRECT_WORD_LINK_COLOR + "\"><a href=\"inputTerm:" + basicWord
                        + "\">" + word + "</a></font> ");
                ...
                builder.append("<font color=\"" + REMOVED_WORD_LINK_COLOR + "\"><a href=\"removedTerm:" + basicWord
                        + "\">" + word + "</a></font> ");
     ...
     * @return
     */
    private int getTermSize(List<String> basicStringList, Set<List<String>> termInfoCompoundWordSet, int i) {
        ...
        if (selectedDoc != null) {
            for (int j = 0; j < selectedDoc.getSize(); j += DISPLAY_LINE_NUM) {
                if (j == lineNum) {
                    ...
                    linkBuilder.append("<font color=" + CORRECT_WORD_LINK_COLOR + "><a href=\"" + j + "\">" + (j + 1)
                            + "-" + (j + DISPLAY_LINE_NUM) + "</a></font><br>");
                }
            }
        }
    ...
    private boolean isRegisteredWord(String word, String type) {
        InputTermSelectionPanel inputTermSelectionPanel = DODDLE.getCurrentProject().getInputTermSelectionPanel();
        if (type.equals("inputTerm")) { return inputTermSelectionPanel.getInputTermInfo(word) != null; }
