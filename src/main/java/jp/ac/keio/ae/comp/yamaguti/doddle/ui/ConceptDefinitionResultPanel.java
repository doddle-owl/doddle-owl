...
public class ConceptDefinitionResultPanel extends JPanel implements ActionListener, ListSelectionListener,
        ...
        try {
            for (NonTaxonomicRelation nonTaxRel : nonTaxRelSet) {
                ...
                String sql = "INSERT INTO concept_definition (Project_ID,is_Meta_Property,Term1,Relation,Term2) "
                        + "VALUES(" + projectID + "," + isMetaProperty + ",'" + domain + "','" + relation + "','"
                        + range + "')";
    ...
    public void loadConceptDefinition(int projectID, Statement stmt) {
        try {
            ...
            while (rs.next()) {
                boolean isMetaProperty = DBManagerDialog.getMySQLBoolean(rs.getInt("is_Meta_Property"));
                String term1 = URLDecoder.decode(rs.getString("Term1"), "UTF8");
                Concept relation = doddleProject.getConstructPropertyPanel().getConcept(rs.getString("Relation"));
    ...
    public void saveWrongPairSet(int projectID, Statement stmt) {
        ...
        try {
            for (WrongPair wp : wrongPairSet) {
                String sql = "INSERT INTO wrong_pair (Project_ID,Term1,Term2) " + "VALUES(" + projectID + ",'"
                        + URLEncoder.encode(wp.getDomain(), "UTF8") + "','" + URLEncoder.encode(wp.getRange(), "UTF8")
                        + "')";
                stmt.executeUpdate(sql);
            }
        } catch (SQLException sqle) {
    ...
    public void addNonTaxonomicRelation(NonTaxonomicRelation nonTaxRel) {
        ...
        if (nonTaxRelSet.contains(nonTaxRel)) { return; } // 定義域，値域，関係すべてが同じ定義は追加できない
    ...
    private Concept getPropertyRootConcept() {
        ...
        if (treeModel.getRoot() instanceof ConceptTreeNode) { return ((ConceptTreeNode) treeModel.getRoot())
                .getConcept(); }
        JOptionPane.showMessageDialog(this, "概念定義には，プロパティ階層の構築が必要です．", "Error", JOptionPane.ERROR_MESSAGE);
        return null;
    }
        ...
        if (conceptDefinitionTable.getSelectedRowCount() == 1) {
            ...
            if (treeModel.getRoot() instanceof ConceptTreeNode) {
                ...
                if (newRelation != null) {
                    ...
                    Concept prevRelation = (Concept) conceptDefinitionTable.getModel().getValueAt(row, 2);
                    String range = (String) conceptDefinitionTable.getModel().getValueAt(row, 3);
                    conceptDefinitionTable.setValueAt(newRelation, row, 2);
                    NonTaxonomicRelation prevNonTaxRel = new NonTaxonomicRelation(domain, prevRelation, range);
                    for (NonTaxonomicRelation nonTaxRel : nonTaxRelSet) {
    ...
    public void tableChanged(TableModelEvent e) {
        ...
        if (row < 0) { return; }
        if (conceptDefinitionTable.getRowCount() <= row) { return; }
