...
public class TermInfoTablePanel extends JPanel implements ActionListener, KeyListener, ListSelectionListener {
    ...
    public TermInfoTablePanel() {
        ...
        searchTermField.setBorder(BorderFactory.createTitledBorder(Translator.getTerm("TermFilterTextField")));
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (isDeletingTableItems) { return; }
        ...
        if (0 <= row) {
            String term = (String) termInfoTable.getValueAt(row, getColumnNamePosition(termInfoTable, Translator
                    .getTerm("TermLabel")));
            String targetLines = DODDLE.getCurrentProject().getDocumentSelectionPanel().getTargetHtmlLines(term);
    ...
    public void saveTermInfoTable(int projectID, Statement stmt, String termTable) {
        String posTable = termTable + "_pos_list";
        String docTable = termTable + "_doc_list";
        DBManagerDialog.deleteTableContents(projectID, stmt, termTable);
        DBManagerDialog.deleteTableContents(projectID, stmt, posTable);
        DBManagerDialog.deleteTableContents(projectID, stmt, docTable);
        if (termInfoMap == null) { return; }
        try {
            ...
            for (TermInfo info : termInfoMap.values()) {
                ...
                String sql = "INSERT INTO " + termTable + " (Project_ID,Term,POS_List_ID,TF,IDF,TF_IDF,Doc_List_ID) "
                        + "VALUES(" + projectID + ",'" + term + "'," + posAndDocListID + "," + tf + "," + idf + ","
                        + tfidf + "," + posAndDocListID + ")";
                stmt.executeUpdate(sql);
                for (String pos : posSet) {
                    sql = "INSERT INTO " + posTable + " (Project_ID,POS_List_ID,POS) VALUES(" + projectID + ","
                            + posAndDocListID + ",'" + pos + "')";
                    stmt.executeUpdate(sql);
                }
                for (File doc : docSet) {
                    ...
                    sql = "INSERT INTO " + docTable + " (Project_ID,Doc_List_ID,Doc,TF) VALUES(" + projectID + ","
                            + posAndDocListID + ",'" + docPath + "'," + inputDocTF + ")";
                    stmt.executeUpdate(sql);
                }
                posAndDocListID++;
            }
    ...
    private void searchTermOrPOS(EventObject e) {
        if (e.getSource() == searchTermField || e.getSource() == searchPOSField) {
            try {
                if (searchTermField.getText().length() == 0 && searchPOSField.getText().length() == 0) {
                    rowSorter.setRowFilter(RowFilter.regexFilter(".*", new int[] { 0}));
                }
                if (searchTermField.getText().length() != 0) {
                    rowSorter.setRowFilter(RowFilter.regexFilter(searchTermField.getText(), new int[] { 0}));
                }
