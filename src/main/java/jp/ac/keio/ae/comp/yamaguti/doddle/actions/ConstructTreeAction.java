...
public class ConstructTreeAction {
    ...
    public ConstructTreeAction(boolean isNounAndVerbTree, DODDLEProject p) {
        ...
        compoundConstructTreeOptionMap = inputConceptSelectionPanel.getCompoundConstructTreeOptionMap();
    ...
    private void addCompoundWordNode(int len, InputTermModel iwModel, TreeNode node) {
        if (len == iwModel.getCompoundWordLength()) { return; }
        List<Token> tokenList = iwModel.getTokenList();
        StringBuilder buf = new StringBuilder();
        boolean isEnglish = isEnglish(iwModel.getTerm());
        for (int i = tokenList.size() - len - 1; i < tokenList.size(); i++) {
            // 食べ放題が「食べる放題」のように活用が原形に変換されるのを防ぐために，getSurface()を使用
            buf.append(tokenList.get(i).getSurface());
            if (isEnglish) {
                buf.append(" ");
            }
        }
    ...
    private boolean hasXCompoundWordChild(TreeNode node) {
        ...
        for (int i = 0; i < node.getChildCount(); i++) {
            ...
            if (childNode.getUserObject() instanceof String) {
                ...
                if (getAbstractConceptChildNum() < compoundWordChildNum) { return false; }
            }
        }
     ...
     */
    private void moveCompoundWordNodeSet(DefaultMutableTreeNode node, Set<DefaultMutableTreeNode> addNodeSet,
     ...
     * @param childNode
     */
    private void extractMoveCompoundWordNodeSet(Set<String> sameNodeSet, Set<DefaultMutableTreeNode> addNodeSet,
    ...
    private void trimCompoundWord(DefaultMutableTreeNode node, DefaultMutableTreeNode childNode, Set addNodeSet,
            Set removeNodeSet) {
        if (childNode.getUserObject() instanceof String && !compoundWordSet.contains(childNode.toString())) {
            for (int i = 0; i < childNode.getChildCount(); i++) {
    ...
    private void setCompoundConcept(CompoundConceptTreeInterface ccTreeInterface, Set<Concept> conceptSet) {
        ...
        DODDLE.getLogger().log(Level.DEBUG, Translator.getTerm("ConstructConceptTreeFromCompoundWordsMessage"));
        ...
        if (project.getInputConceptSelectionPanel().getPartiallyMatchedOptionPanel().isTrimming()) {
            ...
            DODDLE.getLogger().log(Level.DEBUG, Translator.getTerm("TrimmingCompoundWordsMessage"));
        }

        if (project.getInputConceptSelectionPanel().getPartiallyMatchedOptionPanel().isAddAbstractConcept()) {
            addAbstractTreeNode(rootNode);
            trimAbstractNode(rootNode);
            trimLeafAbstractNode();
            DODDLE.getLogger().log(Level.DEBUG, Translator.getTerm("AddAbstractInternalConceptsMessage"));
        }
    ...
    private void reconstructCompoundTree(int d, DefaultMutableTreeNode node) {
        ...
        if (!(node.getUserObject() instanceof Concept)) { // 抽象ノードを上位に持つ複合語は処理しない
            ...
            for (int i = 0; i < node.getChildCount(); i++) {
                DefaultMutableTreeNode childNode = (DefaultMutableTreeNode) node.getChildAt(i);
                if (childNode.getUserObject() instanceof String) {
                    InputTermModel iwModel = inputConceptSelectionPanel.makeInputTermModel(childNode.toString());
                    Set<Concept> correspondConceptSet = wordCorrespondConceptSetMap.get(iwModel.getTopBasicWord());
                    if (correspondConceptSet != null) {
                        for (Concept headConcept : correspondConceptSet) {
                            tmpcnt++;
                            if (headConcept != null && 1 < iwModel.getTokenList().size()) {
                                Set<Concept> supConceptSet = getSupConceptSet(headConcept.getLocalName());
     ...
     * @param headWordBuf
     */
    private void printHeadWordDebug(InputTermModel iwModel, Concept headConcept, int j, StringBuilder headWordBuf) {
        /*
    ...
    private DefaultMutableTreeNode getAbstractNode(DefaultMutableTreeNode node, Map abstractConceptTreeNodeMap,
            DefaultMutableTreeNode childNode, Concept supConcept, InputTermModel iwModel) {
    ...
    private TreeModel makeClassTreeModel(Set<Concept> nounConceptSet) {
        ...
        if (project.getInputConceptSelectionPanel().getConstructionTypePanel().isNewConstruction()) {
            ...
            if (project.getInputConceptSelectionPanel().getPerfectlyMatchedOptionPanel().isConstruction()) {
                ...
                classTreeModel = ConceptTreeMaker.getInstance().getDefaultConceptTreeModel(
                        new HashSet<List<Concept>>(), project, ConceptTreeMaker.DODDLE_CLASS_ROOT_URI);
            }
        ...
        if (project.getInputConceptSelectionPanel().getPartiallyMatchedOptionPanel().isConstruction()) {
        ...
        Set<String> conceptIdSetForReplaceSubConcepts = inputConceptSelectionPanel.getURISetForReplaceSubConcepts();
    ...
    private void replaceSubConcepts(Set<ConceptTreeNode> replaceNodeSet) {
        ...
        for (int i = 0; i < replaceNodes.length; i++) {
            ...
            while (replaceNode.getChildCount() != 0) {
                DefaultMutableTreeNode childNode = (DefaultMutableTreeNode) replaceNode.getChildAt(0);
    ...
    private void checkReplaceSubConcepts(ConceptTreeNode node, Set<String> conceptIdSetForReplaceSubConcepts,
            Set<ConceptTreeNode> replaceNodeSet) {
        for (int i = 0; i < node.getChildCount(); i++) {
    ...
    private TreeModel makePropertyTreeModel(Set<Concept> verbConceptSet) {
        ...
        if (project.getInputConceptSelectionPanel().getConstructionTypePanel().isNewConstruction()) {
            ...
            if (project.getInputConceptSelectionPanel().getPerfectlyMatchedOptionPanel().isConstruction()) {
                propertyTreeModel = constructPropertyPanel.getTreeModel(constructClassPanel.getAllConceptURI(),
                        verbConceptSet, ConceptTreeMaker.DODDLE_PROPERTY_ROOT_URI);
            } else {
                propertyTreeModel = ConceptTreeMaker.getInstance().getDefaultConceptTreeModel(
                        new HashSet<List<Concept>>(), project, ConceptTreeMaker.DODDLE_PROPERTY_ROOT_URI);
            }
        ...
        Set<ConceptTreeNode> replaceNodeSet = new HashSet<ConceptTreeNode>();
        Set<String> uriSetForReplaceSubConcepts = inputConceptSelectionPanel.getURISetForReplaceSubConcepts();
     ...
     */
    private void getUnnecessaryAbstractNode(ConceptTreeNode node, Set<ConceptTreeNode> unnecessaryNodeSet) {
        for (int i = 0; i < node.getChildCount(); i++) {
            ConceptTreeNode childNode = (ConceptTreeNode) node.getChildAt(i);
    ...
    public void constructTree() {
        ...
        } else if (project.getInputConceptSelectionPanel().getPerfectlyMatchedOptionPanel().isConstruction()) {
            JOptionPane.showMessageDialog(DODDLE.getCurrentProject(), Translator.getTerm("NotImplementedMessage"),
        ...
        DODDLE.getLogger().log(Level.INFO,
                Translator.getTerm("InputTermCountMessage") + ": " + (inputConceptSelectionPanel.getMatchedTermCnt()));
        DODDLE.getLogger().log(Level.INFO,
        ...
        if (project.getInputConceptSelectionPanel().isConstructNounAndVerbTree()) {
            ...
            DODDLE.getLogger().log(Level.INFO,
                    Translator.getTerm("InputNounConceptCountMessage") + ": " + inputNounConceptSet.size());
            DODDLE.STATUS_BAR.addValue();
            ...
            TreeModel hasaTreeModel = ConceptTreeMaker.getInstance().getDefaultConceptTreeModel(
                    new HashSet<List<Concept>>(), project, ConceptTreeMaker.DODDLE_CLASS_HASA_ROOT_URI);
            constructClassPanel.setHasaTreeModel(hasaTreeModel);
            DODDLE.STATUS_BAR.addValue();
            DODDLE.getLogger().log(Level.INFO,
                    Translator.getTerm("InputVerbConceptCountMessage") + ": " + inputVerbConceptSet.size());
            constructPropertyPanel.setConceptTreeModel(makePropertyTreeModel(inputVerbConceptSet));
            hasaTreeModel = ConceptTreeMaker.getInstance().getDefaultConceptTreeModel(new HashSet<List<Concept>>(),
                    project, ConceptTreeMaker.DODDLE_PROPERTY_HASA_ROOT_URI);
            ...
            constructClassPanel.setHasaTreeModel(hasaTreeModel);
            DODDLE.STATUS_BAR.addValue();
            constructPropertyPanel.setConceptTreeModel(makePropertyTreeModel(new HashSet<Concept>()));
            hasaTreeModel = ConceptTreeMaker.getInstance().getDefaultConceptTreeModel(new HashSet<List<Concept>>(),
                    project, ConceptTreeMaker.DODDLE_PROPERTY_HASA_ROOT_URI);
            constructPropertyPanel.setHasaTreeModel(hasaTreeModel);
            DODDLE.STATUS_BAR.addValue();
        ...
        DODDLE.setSelectedIndex(DODDLEConstants.TAXONOMIC_PANEL);
        DODDLE.STATUS_BAR.printMessage(Translator.getTerm("TreeConstructionDoneMessage"));
        DODDLE.STATUS_BAR.addValue();
        DODDLE.STATUS_BAR.hideProgressBar();
    }
}
